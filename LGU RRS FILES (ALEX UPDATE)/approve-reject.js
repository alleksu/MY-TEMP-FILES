...
function renderApprove(data) {
  ...
  tbody.innerHTML = data.length === 0
    ? '<tr><td colspan="7" style="text-align:center;color:var(--text-3);padding:24px">No requests found.</td></tr>'
    : data.map(r => {
        ...
        return `<tr>
          ...
          <td style="white-space:nowrap">
            ...
            <button class="action-view-btn"    onclick="viewRecord(${i})"  title="View">
...
function viewRecord(idx) {
  ...
  document.getElementById('modal-record-content').innerHTML = `
    ...
    <div class="receipt-row"><span>Issued By:</span><span>Officer Manansala</span></div>
  `;
...
function confirmApprove() {
  records[approveTarget].status = 'Posted';
...
function confirmReject() {
  records[rejectTarget].status = 'Rejected';
